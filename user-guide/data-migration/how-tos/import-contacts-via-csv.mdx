...
description: Complete step-by-step guide to importing people/contacts into Clara.
---

## Overview

This guide walks you through importing your contacts (People) into Clara. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in Clara
- For additional emails, use the **Emails / Additional Emails** column:
```
["jane@get-clara.tech","jane.doe@get-clara.tech"]
```
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in Clara.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names Clara expects.
</Note>
...
3. Wait for Clara to analyze your file

## Step 4: Map Your Columns

Clara automatically tries to match your columns to fields. Review and adjust:
...
3. Clara will link each Person to the matching Company
...
1. Clara shows your values alongside existing options
2. Match each value in your file to a Clara option
...
Before completing the import, Clara validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in Clara or file | Remove duplicate or update existing record |
...
Your contacts are now in Clara! Next steps:
...
2. Clara matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in Clara. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for Clara's internal ID.
  </Accordion>
