...
description: Step-by-step guide to migrate your data from any CRM to Clara.
---

## Overview

This guide walks you through migrating your data from any CRM to Clara. The process involves auditing your data, preparing your Clara workspace, exporting from your current system, and importing into Clara.
...
Create a mapping document between your current CRM and Clara:
...
## Step 3: Set Up Your Clara Workspace

Before importing data, prepare your Clara workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Clara before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Clara.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Clara already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Clara's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Clara
...
- Clara will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Clara.
  </Accordion>
...
Contact [contact@get-clara.tech](mailto:contact@get-clara.tech) or explore our [Implementation Services](/user-guide/getting-started/capabilities/implementation-services).
