...
The parent ("one" side) must exist in Clara before you can reference it.
...
If you need the `id`:
1. Export the parent records from Clara
...
Use `https://domain.com` (not just `domain.com`). This matches how Clara stores Company domains and prevents matching errors.
</Warning>
...
Clara will automatically link each child record to the matching parent.
...
| **Non-existent records** | The parent record doesn't exist in Clara | Import parent records first, or check for typos |
| **Case mismatch** | `Acme.com` in file but `acme.com` in Clara | Ensure exact case matching |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the Clara `id` (export parent records to get their IDs)
