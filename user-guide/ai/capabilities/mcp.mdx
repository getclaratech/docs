---
title: MCP Server
description: Connect AI assistants to your Clara workspace using the Model Context Protocol.
---
...
Clara exposes an [MCP](https://modelcontextprotocol.io/) server so that AI assistants — Claude Desktop, Claude Code, Cursor, ChatGPT, and others — can read and write your CRM data through natural language.

Use your **workspace URL** (the URL you use to access Clara) as the MCP endpoint. On Clara Cloud, your workspace URL might be `https://app.get-clara.tech` or a custom domain. The server is available at:

| Environment | MCP Endpoint |
|-------------|-------------|
| **Cloud** | `https://{your-workspace-url}/mcp` (e.g. `https://app.get-clara.tech/mcp`) |
...
Add this to your MCP client configuration, replacing `{your-workspace-url}` with your workspace host:
...
1. Discover Clara's OAuth metadata via `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`
...
Go to **Settings > AI > More > MCP Server** in Clara. Choose your authentication method (OAuth or API Key), copy the JSON snippet (it will already use your workspace URL), and paste it into your MCP client's config file.
...
Restart your MCP client (or reload the config). If using OAuth you will be redirected to Clara to authorize access. If using an API key the connection is immediate.
...
Once connected, the MCP server exposes tools that mirror the Clara API. The recommended workflow is:
...
For self-hosted instances, replace `{your-workspace-url}` with your server URL. Make sure `SERVER_URL` in your environment matches the public URL of your Clara instance — this is used to generate the OAuth discovery metadata.
