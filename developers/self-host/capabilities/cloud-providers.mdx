---
title: Other methods
icon: "cloud"
---

<Warning>
  This document is maintained by the community. It might contain issues.
</Warning>

## Kubernetes via Terraform and Manifests

Community-led documentation for Kubernetes deployment is available [here](https://github.com/getclaratech/clara/tree/main/packages/twenty-docker/k8s)

### Coolify

Deploy Clara on servers using Coolify. (official image on Coolify will be available soon)

[Coolify documentation](https://coolify.io/docs/get-started/introduction)


### EasyPanel

Deploy Clara on EasyPanel with the community maintained template below.

[Deploy on EasyPanel](https://easypanel.io/docs/templates/twenty)

### Elest.io

Deploy Clara on servers with Elest.io using link below.

[Deploy on Elest.io](https://elest.io/open-source/twenty)

### Clara on Railway

Deploy Clara on Railway with the community maintained template below.

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/nAL3hA)

### Clara on Sealos

Deploy Clara on Sealos with the community maintained template below.

[![Deploy on Sealos](https://sealos.io/Deploy-on-Sealos.svg)](https://sealos.io/products/app-store/twenty)

## Others

Please feel free to Open a PR to add more Cloud Provider options.

