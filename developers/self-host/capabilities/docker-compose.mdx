...
This guide provides step-by-step instructions to install and configure the Clara application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of Clara with a single command:
```bash
bash <(curl -sL https://raw.githubusercontent.com/getclaratech/clara/main/packages/twenty-docker/scripts/install.sh)
```

To install a specific version or branch:
```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/getclaratech/clara/main/packages/twenty-docker/scripts/install.sh)
```
...
1. **Create the .env File**
    ...
    ```bash
    curl -o .env https://raw.githubusercontent.com/getclaratech/clara/refs/heads/main/packages/twenty-docker/.env.example
    ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/getclaratech/clara/refs/heads/main/packages/twenty-docker/docker-compose.yml
```
...
### Expose Clara to External Access

By default, Clara runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting Clara on a regular domain.
...
We strongly recommend setting up Clara behind a reverse proxy with SSL termination for optimal security and functionality.
