...
Logic functions are server-side TypeScript functions that run on the Clara platform. They can be triggered by HTTP requests, cron schedules, or database events — and can also be exposed as tools for AI agents.
...
Logic functions can be exposed as **tools** for AI agents and workflows. When marked as a tool, a function becomes discoverable by Clara's AI features and can be used in workflow automations.
...
- The environment variables `APPLICATION_ID`, `APP_ACCESS_TOKEN`, and `API_URL` are available inside the handler (same as any other logic function), so you can call the Clara API with an application access token scoped to your app.
...
The `twenty-client-sdk` package provides two typed GraphQL clients for interacting with the Clara API from your logic functions and front components.
...
<Note>
  When your code runs on Clara (logic functions or front components), the platform injects credentials as environment variables:

  - `TWENTY_API_URL` — Base URL of the Clara API
