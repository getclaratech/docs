---
title: Architecture
description: How Clara apps work — sandboxing, lifecycle, and the building blocks.
icon: "sitemap"
---

Clara apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the Clara platform with full sandboxing and permission controls.
...
| **Front Component** | Sandboxed React UI in Clara's page | [Front Components](/developers/extend/apps/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with Clara via a message-passing host API.
...
├─────────────────────────────────────────────────────────┤
│ Publish                                                 │
│   npm publish → appears in Clara marketplace            │
└─────────────────────────────────────────────────────────┘
```

- **`yarn twenty dev`** — watches your source files and live-syncs changes to a connected Clara server. The typed API client is regenerated automatically when the schema changes.
...
<CardGroup cols={2}>
  ...
  <Card title="Front Components" icon="window-maximize" href="/developers/extend/apps/front-components">
    Sandboxed React components inside Clara's UI.
  </Card>
