...
title: Publishing
icon: "upload"
description: Distribute your Clara app to the marketplace or deploy it internally.
---
...
- **Deploy a tarball** — upload your app directly to a specific Clara server for internal or private use.
- **Publish to npm** — list your app in the Clara marketplace for any workspace to discover and install.
...
For apps you don't want publicly available — proprietary tools, enterprise-only integrations, or experimental builds — you can deploy a tarball directly to a Clara server.
...
2. Spawns an isolated Clara test instance using the `getclaratech/clara/.github/actions/spawn-twenty-app-dev-test@main` composite action (the CI equivalent of `yarn twenty server start --test`).
...
- `TWENTY_VERSION` (env, defaults to `latest`) — pin the Clara server version used in CI by editing this in `ci.yml`.
...
Deploys your app to a configured Clara server on every push to `main`, and optionally from a pull request when the `deploy` label is applied.

**What it does:**

1. Checks out the PR head (for labeled PRs) or the pushed commit.
2. Runs `getclaratech/clara/.github/actions/deploy-twenty-app@main` — the CI equivalent of `yarn twenty deploy`.
3. Runs `getclaratech/clara/.github/actions/install-twenty-app@main` so the newly deployed version is installed into the target workspace.

**Required configuration:**

| Setting | Where | Purpose |
|---------|-------|---------|
| `TWENTY_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:3000`) | The Clara server to deploy to. Change this to your real server URL before first use. |
...
Both workflows reference reusable actions at `@main`, so action updates in the `getclaratech/clara` repo are picked up automatically. If you want deterministic builds, replace `@main` with a commit SHA or release tag on each `uses:` line.

## Publishing to npm

Publishing to npm makes your app discoverable in the Clara marketplace. Any Clara workspace can browse, install, and upgrade marketplace apps directly from the UI.
...
The Clara server syncs its marketplace catalog from the npm registry **every hour**.
...
<Note>
If your app does not define an `aboutDescription` in `defineApplication()`, the marketplace will automatically use your package's `README.md` from npm as the about page content. This means you can maintain a single README for both npm and the Clara marketplace. If you want a different description in the marketplace, explicitly set `aboutDescription`.
</Note>
...
Go to the **Settings > Applications** page in Clara, where both marketplace and tarball-deployed apps can be browsed and installed.
