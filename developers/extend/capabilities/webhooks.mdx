...
Webhooks push data to your systems in real-time when events occur in Clara — no polling required. Use them to keep external systems in sync, trigger automations, or send alerts.
...
Clara sends webhooks for these event types:
...
Clara signs each webhook request for security. Validate signatures to ensure requests are authentic.
